defmodule Nspark.PromptBasic.Analyzer do
  @moduledoc """
  Static lens over a parsed PromptBasic program — the X-ray.

  Returns a list of `{severity, title, lines}` findings. Severities:
  `:error` (🔴), `:warn` (🟡), `:smell` (🟠), `:info` (🔎), `:ok` (✅).

  Sound checks (deterministic, zero-LLM, CI-gateable): dead states, dead/write-only
  memory, tool inventory, output-reference resolution, priority ties, unreachable rules.
  Heuristic checks (advisory `:warn` only, never a hard gate): priority preemption,
  complementary modeling smell. See `docs/pivot/DESIGN_MEMO_two_layer_lens.md` §5 for the
  soundness boundary. Promoted from `docs/pivot/lens_analyzer.exs`.
  """
  ...
  defp dead_memory(prog, rules) do
    ...
    read =
      rules
      |> Enum.flat_map(& &1.atoms)
      |> Enum.filter(&(&1.kind == :memory and &1.key != "state"))
      |> Enum.map(& &1.key)
      |> MapSet.new()
    ...
    findings =
      Enum.map(MapSet.to_list(dead), fn k ->
        "🟠 `#{k}` declared but never read or written — dead memory"
      end) ++
        Enum.map(MapSet.to_list(write_only), fn k ->
          "🟡 `#{k}` written but no [WHEN] ever reads it"
        end)
  ...
  defp unresolved_refs(rules) do
    findings =
      Enum.flat_map(rules, fn r ->
        ...
        |> Enum.map(
          &"🔴 #{r.id}: {{tool:#{&1.var}}} referenced but no tool in this rule outputs `#{&1.var}`"
        )
      end)
      ...
      ts ->
        {:info, "PRIORITY TIES",
         Enum.map(ts, fn {p, rs} ->
           ...
           verdict =
             if mutually_exclusive_group?(rs),
               do: "OK (conditions mutually exclusive)",
               else: "⚠ ambiguous"
  ...
  defp shadowing(rules) do
    findings =
      for low <- rules, high <- rules, low.id != high.id, num(low.priority) < num(high.priority) do
        if subset?(keyset(high), keyset(low)) and keyset(high) != MapSet.new() do
          "🔴 #{low.id} (P#{low.priority}) is unreachable — every input matching it also matches " <>
            "#{high.id} (P#{high.priority}), which wins"
        end
      end
    ...
    case findings do
      [] ->
        {:ok, "UNREACHABLE RULES",
         ["none (no rule is fully shadowed by a higher-priority generalisation)"]}

      fs ->
        {:error, "UNREACHABLE RULES", fs}
    end
  end
  ...
  defp drain_note do
    {:info, "COVERAGE DRAIN",
     [
       ...
       "statically — intended-behavior gaps need the Test layer (Selector), not the parser."
     ]}
  end
